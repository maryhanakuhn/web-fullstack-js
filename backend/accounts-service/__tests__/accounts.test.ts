...
describe("Testando rotas do accounts", () => {
  ...
  it("PATCH /accounts/:id - Deve retornar statusCode 200", async () => {
    ...
    const resultado = await request(app).patch("/accounts/" + testId).send(payload).set("x-access-token", jwt);
  ...
  it("PATCH /accounts/:id - Deve retornar statusCode 400", async () => {
    ...
    const resultado = await request(app).patch("/accounts/abc").send(payload).set("x-access-token", jwt);
  ...
  it("GET /accounts/:id - Deve retornar statusCode 200", async () => {
    const resultado = await request(app).get("/accounts/"+testId).set("x-access-token", jwt);
