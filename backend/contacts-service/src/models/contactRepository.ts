...
async function findById(contactId: number, accountId: number) {
  return contactModel.findOne<IContactModel>({
    where: { id: contactId, accountId: accountId },
  });
}
...
export default { findAll, add, set, removeByEmail, removeById, findById };
